...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
	// - 1. Allocate matrix to DEVICE GPU deive memory
	Matrix M_Device;
	Matrix N_Device;
	Matrix P_Device;
	M_Device = AllocateDeviceMatrix(M);
	N_Device = AllocateDeviceMatrix(N);
	P_Device = AllocateDeviceMatrix(P);

	// - 2. LOAD (via memcpy) matricies to GPU memory
	CopyToDeviceMatrix(M_Device, M);
	CopyToDeviceMatrix(N_Device, N);

	// - 3. Kernel Invocation
	dim3 dimBlock(16, 16); //matrix size is 16x16 for threads
	dim3 dimGrid(1,1); //1x1 grid for simple matrix multiplciation
	MatrixMulKernel<<<dimGrid, dimBlock>>>(M_Device, N_Device, P_Device);
	
	// - 4. Copy result from GPU to host
	CopyFromDeviceMatrix(P, P_Device);

	
	// -5. Free memory
	cudaFree(M_Device.elements);
	cudaFree(N_Device.elements);
 	cudaFree(P_Device.elements);
